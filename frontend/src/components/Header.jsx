...
export default function Header({
  ...
  // 👇 Usamos el carrito para saber cuántos items hay y abrirlo
            ...
            <div className="relative fondo-plateado h-full overflow-y-auto pt-14 p-5" style={{ backgroundColor: "#000" }}>
              ...
              {user ? (
                <>
                  ...
                  <button onClick={() => { onLogout(); setSidebarOpen(false); }} className="w-full fondo-plateado text-left mt-2 px-4 py-2 rounded-lg font-semibold text-red-700">Cerrar sesión</button>
                </>
              ) : (
                <button onClick={() => { onLoginClick(); setSidebarOpen(false); }} className="w-full fondo-plateado text-left px-4 py-2 rounded-lg font-semibold">Iniciar sesión</button>
              )}
            </div>
          </div>
        </div>
